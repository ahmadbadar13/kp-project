...
const SubBagianPDI_Adm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/sub-bagian-pdi-adm');
  ...
  const handleCommentSubmit = async () => {
    try {
      ...
      await axios.post('http://localhost:5000/api/tambah-komentar-sb-pdi', {
                  ...
                  <div className="relative bg-white rounded-md shadow-lg p-6 w-11/12 max-w-lg">
                    ...
                    <div className="text-center">
                      <img
                        ...
                        alt={`Foto ${selectedUser.nama_sb_pdi}`}
                        className="w-32 h-32 rounded-full mx-auto mb-4"
                      />
                      <h2 className="text-xl font-semibold mb-2">
                        {selectedUser.nama_sb_pdi}
                      </h2>
                      <p className="text-lg font-medium">
                        {/^\d+$/.test(selectedUser.nip_sb_pdi)
                          ? `NIP: ${selectedUser.nip_sb_pdi}`
                          : "PPNPN"}
                      </p>
                      <p className="text-lg font-medium">
                        Posisi: {selectedUser.posisi_sb_pdi}
                      </p>
                      <p className="text-lg font-medium">
...
export default SubBagianPDI_Adm;
