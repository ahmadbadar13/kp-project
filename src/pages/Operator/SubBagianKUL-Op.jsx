...
const SubBagianKUL_Op = () => {
  ...
  const handleSubmitNewUser = async (e) => {
    ...
    const formData = {
        ...
        komentar_sb_kul: newUser.komentar_sb_kul || null // Menangani null
    };
    ...
    try {
        ...
        const response = await axios.post('http://localhost:5000/api/sub-bagian-kul-op', formData);

        if (response.data.success) {
            Swal.fire('Berhasil!', 'Data baru telah ditambahkan.', 'success');
            setNewUser({ subbagian: '', komentar_sb_kul: '' });
...
const fetchUsers = async () => {
  try {
    const response = await axios.get('http://localhost:5000/api/sub-bagian-kul-op');
  ...
  const fetchComments = async (userId) => {
    try {
        const response = await axios.get(`http://localhost:5000/api/komentar-sb-kul/${userId}`);
  ...
  const deleteComment = async (userId) => {
      try {
          await axios.delete(`http://localhost:5000/api/komentar-sb-kul/${userId}`);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col min-h-screen p-6">
        <h1 className="text-2xl font-bold mb-6 text-center">Data Pegawai Sub Bagian Keuangan, Umum dan Logistik</h1>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div className="relative bg-white rounded-md shadow-lg p-6 w-11/12 max-w-lg">
                    ...
                    <div className="text-center">
                      <img
                        src={`http://localhost:5000${selectedUser.foto_sb_kul}`}
                        alt={selectedUser.nama_sb_kul}
