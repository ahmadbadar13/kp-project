...
const DivisiPDI_Op = () => {
  ...
  const [periode, setPeriode] = useState({ id_divisi_pdi: '', periode_awal: '', periode_akhir: '' });

  const handleAddPeriod = () => setIsAddingPeriod(true);
  const handleCancelAddPeriod = () => {
    setIsAddingPeriod(false);
    setPeriode({ id_divisi_pdi: '', periode_awal: '', periode_akhir: '' });
  };
  ...
  const handleSubmitNewUser = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/divisi-pdi-op', formData);
  ...
  const handleSubmitEditUser = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/divisi-pdi-op/${editingUser.id}`, formData);
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/divisi-pdi-op');
  ...
  const fetchComments = async (userId) => {
    try {
        const response = await axios.get(`http://localhost:5000/api/komentar-divisi-pdi/${userId}`);
  ...
  const deleteComment = async (userId) => {
      try {
          await axios.delete(`http://localhost:5000/api/komentar-divisi-pdi/${userId}`);
  ...
  const handleSubmitPeriod = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/divisi-pdi/add-periode', periode);
        ...
        <div>
          ...
          >
            ...
            <form onSubmit={handleSubmitPeriod}>
              <div className="mb-4">
                <label htmlFor="id_divisi_pdi" className="block text-gray-700">Divisi PDI</label>
                <select
                  id="id_divisi_pdi"
                  value={periode.id_divisi_pdi}
                  onChange={(e) => setPeriode({ ...periode, id_divisi_pdi: e.target.value })}
