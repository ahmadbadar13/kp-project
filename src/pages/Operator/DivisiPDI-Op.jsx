...
const DivisiPDI_Op = () => {
  ...
  const fetchComments = async (userId) => {
    try {
        const response = await axios.get(`http://localhost:5002/api/komentar-divisi-pdi/${userId}`);
  ...
  const deleteComment = async (userId) => {
      try {
          await axios.delete(`http://localhost:5002/api/komentar-divisi-pdi/${userId}`);
