  ...
  const handleSubmitNewUser = async (e) => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            setNewUser({ subbagian: '', komentar_sb_hsdm: '' });
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        await axios.delete(`http://localhost:5000/api/sub-bagian-hsdm-op/anggota/${userId}`);
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div className="relative bg-white rounded-md shadow-lg p-6 w-11/12 max-w-lg">
                    ...
                    <div className="text-center">
                      <img
                        src={`http://localhost:5000${selectedUser.foto_sb_hsdm}`}
                        alt={selectedUser.nama_sb_hsdm}
                        className="w-32 h-32 rounded-full mx-auto mb-4"
                      />
                      <h2 className="text-xl font-semibold mb-2">{selectedUser.nama_sb_hsdm}</h2>
                      <p className="text-lg font-medium">NIP: {selectedUser.nip_sb_hsdm}</p>
                      <p className="text-lg font-medium">Posisi: {selectedUser.posisi_sb_hsdm}</p>
