...
const DivisiTP_Op = () => {
  ...
  const [periode, setPeriode] = useState({ id_divisi_tp: '', periode_awal: '', periode_akhir: '' });

  const handleAddPeriod = () => setIsAddingPeriod(true);
  const handleCancelAddPeriod = () => {
    setIsAddingPeriod(false);
    setPeriode({ id_divisi_tp: '', periode_awal: '', periode_akhir: '' });
  };
  ...
  const handleSubmitEditUser = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/divisi-tp-op/${editingUser.id}`, formData);
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/divisi-tp-op');
  ...
  const fetchComments = async (userId) => {
    try {
        const response = await axios.get(`http://localhost:5000/api/komentar-divisi-tp/${userId}`);
  ...
  const deleteComment = async (userId) => {
      try {
          await axios.delete(`http://localhost:5000/api/komentar-divisi-tp/${userId}`);
  ...
  const handleSubmitPeriod = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/divisi-tp/add-periode', periode);
        ...
        <div>
          ...
          >
            ...
            <form onSubmit={handleSubmitPeriod}>
              <div className="mb-4">
                <label htmlFor="id_divisi_tp" className="block text-gray-700">Divisi TP</label>
                <select
                  id="id_divisi_tp"
                  value={periode.id_divisi_tp}
                  onChange={(e) => setPeriode({ ...periode, id_divisi_tp: e.target.value })}
              ...
              {users.map((user) => (
                <div key={user.id} className="bg-gray-200 shadow-md rounded-md p-4 flex flex-col items-center relative">
                  ...
                  <div className="w-32 h-32 mb-4 overflow-hidden rounded-full flex items-center justify-center">
                    <img
                      src={"http://localhost:5000" + user.foto_div_tp}
                      alt={user.nama_div_tp}
