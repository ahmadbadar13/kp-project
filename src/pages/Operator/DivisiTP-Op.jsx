...
const DivisiTP_Op = () => {
  ...
  const handleSubmitNewUser = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/api/divisi-tp-op', formData);
  ...
  const handleSubmitEditUser = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5002/api/divisi-tp-op/${editingUser.id}`, formData);
      if (response.data.success) {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      await axios.delete(`http://localhost:5002/api/divisi-tp-op/${userId}`);
      fetchUsers();
    } catch (error) {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5002/api/divisi-tp-op');
  ...
  const fetchComments = async (userId) => {
    try {
        const response = await axios.get(`http://localhost:5002/api/komentar-divisi-tp/${userId}`);
  ...
  const deleteComment = async (userId) => {
      try {
          await axios.delete(`http://localhost:5002/api/komentar-divisi-tp/${userId}`);
      ...
      {menuOpen && (
        <div className="md:hidden fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-75 z-50">
          <div className="flex flex-col items-center pt-10">
            <Link to="/DivisiTP-Op" onClick={closeMenu} className="text-white text-2xl mb-6">≡</Link>
              ...
              {users.map((user) => (
                <div key={user.id} className="bg-gray-200 shadow-md rounded-md p-4 flex flex-col items-center relative">
                  ...
                  <div className="w-32 h-32 mb-4 overflow-hidden rounded-full flex items-center justify-center">
                    <img
                      src={"http://localhost:5002" + user.foto_div_tp}
