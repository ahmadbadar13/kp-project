...
const DivisiHP_Op = () => {
  ...
  const [periode, setPeriode] = useState({ id_divisi_hp: '', periode_awal: '', periode_akhir: '' });

  const handleAddPeriod = () => setIsAddingPeriod(true);
  const handleCancelAddPeriod = () => {
    setIsAddingPeriod(false);
    setPeriode({ id_divisi_hp: '', periode_awal: '', periode_akhir: '' });
  };
  ...
  const handleSubmitPeriod = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/divisi-hp/add-periode', periode);
      ...
      <div>
        ...
        >
          ...
          <form onSubmit={handleSubmitPeriod}>
            <div className="mb-4">
              <label htmlFor="id_divisi_hp" className="block text-gray-700">Divisi HP</label>
              <select
                id="id_divisi_hp"
                value={periode.id_divisi_hp}
                onChange={(e) => setPeriode({ ...periode, id_divisi_hp: e.target.value })}
