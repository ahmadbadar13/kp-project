...
const DivisiKURL_Op = () => {
  ...
  const [periode, setPeriode] = useState({ id_divisi_kurl: '', periode_awal: '', periode_akhir: '' });

  const handleAddPeriod = () => setIsAddingPeriod(true);
  const handleCancelAddPeriod = () => {
    setIsAddingPeriod(false);
    setPeriode({ id_divisikurl: '', periode_awal: '', periode_akhir: '' });
  };
  ...
  const handleSubmitPeriod = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/divisi-kurl/add-periode', periode);

      if (response.data.success) {
        Swal.fire('Berhasil!', 'Periode berhasil ditambahkan.', 'success');
        setIsAddingPeriod(false);
        setPeriode({ id_divisi_kurl: '', periode_awal: '', periode_akhir: '' });
      } else {
        ...
        <div>
          ...
          >
            ...
            <form onSubmit={handleSubmitPeriod}>
              <div className="mb-4">
                <label htmlFor="id_divisi_kurl" className="block text-gray-700">Divisi KURL</label>
                <select
                  id="id_divisi_kurl"
                  value={periode.id_divisi_kurl}
                  onChange={(e) => setPeriode({ ...periode, id_divisi_kurl: e.target.value })}
