...
const DivisiKURL_Op = () => {
  ...
  const handleSubmitNewUser = async (e) => {
    ...
    const formData = {
        nama_div_kurl: selectedDivision.nama_div,
        foto_div_kurl: selectedDivision.foto_div,
        tanggal_lahir: selectedDivision.tanggal_lahir,
        email: selectedDivision.email,
        komentar_div_kurl: selectedDivision.komentar_div_kurl
    };
  ...
  const handleEditUser = (user) => {
    ...
    setEditingUser({
      id: user.id,
      name: user.nama_div_kurl,
      photo: user.foto_div_kurl,
      tanggal_lahir: user.tanggal_lahir,
      email: user.email,
      komentar_div_kurl: user.komentar_div_kurl
    });
  };
  ...
  const handleSubmitEditUser = async (e) => {
    ...
    formData.append('komentar_div_kurl', editingUser.komentar_div_kurl);
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/divisi-kurl-op/${editingUser.id}`,
        formData,
        {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div className="relative bg-white rounded-md shadow-lg p-6 w-11/12 max-w-lg">
                    ...
                    <div className="text-center">
                      <img
                        src={`http://localhost:5000${selectedUser.foto_div_kurl}`}
                        alt={selectedUser.nama_div_kurl}
                        className="w-32 h-32 rounded-full mx-auto mb-4"
                      />
                      <h2 className="text-xl font-semibold mb-2">{selectedUser.nama_div_kurl}</h2>
