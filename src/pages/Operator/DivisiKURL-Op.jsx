...
const DivisiKURL_Op = () => {
  ...
  const handleEditUser = (user) => {
    ...
    setEditingUser({
      ...
      nip: user.nip_div_kurl,
      position: user.posisi_div_kurl,
  ...
  const handleSubmitNewUser = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/divisi-kurl-op', formData);
  ...
  const handleSubmitEditUser = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5001/api/divisi-kurl-op/${editingUser.id}`, formData);
      if (response.data.success) {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      await axios.delete(`http://localhost:5001/api/divisi-kurl-op/${userId}`);
      fetchUsers();
    } catch (error) {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5001/api/divisi-kurl-op');
              ...
              {users.map((user) => (
                <div key={user.id} className="bg-gray-200 shadow-md rounded-md p-4 flex flex-col items-center ">
                  <div className="w-32 h-32 mb-4 overflow-hidden rounded-full flex items-center justify-center">
                    <img
                      src={"http://localhost:5001" + user.foto_div_kurl}
